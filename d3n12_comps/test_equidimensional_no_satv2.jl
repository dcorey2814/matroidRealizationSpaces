...
include(joinpath(currentDir, "src/matroid_realizationv2.jl"));
include(joinpath(currentDir, "src/reduceIdealsv3.jl"));
...
filename_big = joinpath(min_prime_dir, string("toobig.",ARGS[2],".dat"))
...
io_small = open(filename_equidimensional, "a")
io_big = open(filename_big, "a")



function is_small_ideal(Igens)
    varsIgens = unique!(vcat([vars(f) for f in Igens]...))
    length(varsIgens) >= 4 && return false
    length(varsIgens) <= 2 && return true
    d = maximum([total_degree(f) for f in Igens])
    if d <= 4 return true else return false end
end
...
for z in parse(Int64, ARGS[3]):length(d3n12)
    ...
    MRS = matroid_with_chart_to_reduced_expression(Mz, A, QQ) 

    if !MRS.representable 
        write(io_one, Mzstr, "\n")
        continue
    end
