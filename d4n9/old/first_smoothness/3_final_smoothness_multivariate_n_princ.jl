...
for t in 13217:length(npmult) 
    ...
    if length(ideal_vars(I[1]))>3
        ...
        open("3_to_recheck_nprinc_mult_singular_4_9.dat", "a") do file
                write(file,data,"\n")
            end
    else
        ...
        J = simplified_2_singular_locus(I[1], I[2])
            if !(is_one(J))
                println("                  singular")
                open("3_final_nprinc_mult_singular_4_9.dat", "a") do file
                    write(file,data,"\n")
                end
            else
                println("                  smooth")
                open("3_final_nprinc_mult_smooth_4_9.dat", "a") do file
                     write(file,data,"\n")
                end
            end
