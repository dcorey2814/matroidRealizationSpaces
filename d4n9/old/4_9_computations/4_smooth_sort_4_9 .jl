...
#classify if realizable
    elseif (length(I[1]) == 0||I[1] == [0]) #reduces to 0 
            println("zero ideal")
            open("zero_ideal_4_9/reduce_to_zero_4_9_4.dat", "a") do file
            write(file,data,"\n")
        end   
    ...
    elseif int_gens(I[1])#has integral generator. This only matters if reducing over ZZ
            ...
            if !(isone(J))
                 println("int gen singular")
                    open("int_generator_4_9/singular_4.dat", "a") do file
                         write(file,data,"\n")
                 end 
            else
                println("int gen smooth")
                    open("int_generator_4_9/smooth_4.dat", "a") do file
                         write(file,data,"\n")
                    end
             end
                 ...
                 #println("not principal univariate")#univariate ideal with multiple generators
                   ...
                   if !(isone(J))
                     println("not principle univariate singular")
                        open("np_univariate_4_9/singular_4.dat", "a") do file
                             write(file,data,"\n")
                          end 
                    else
                     println("not principle univariate smooth")
                      open("np_univariate_4_9/smooth_4.dat", "a") do file
                          write(file,data,"\n")
                        end
                    end
        else    
            #println("not principle multivariate")#multivariate ideal with multiple generators
             ...
             if !(isone(J))
                 println("np multivariate singular")
                    open("nprinc_multivariate_4_9/singular_4.dat", "a") do file
                         write(file,data,"\n")
                    end 
            else
                println("not principal multivariate smooth")
                    open("nprinc_multivariate_4_9/smooth_4.dat", "a") do file
                         write(file,data,"\n")
                    end
             end
        end
