    ...
    elseif int_gens(I[1])#has integral generator. This only matters if reducing over ZZ
            ...
            if !(isone(J))
                 println("int gen singular")
                    open("int_generator_3_10/singular.dat", "a") do file
                         write(file,data,"\n")
                 end 
            else
                println("int gen smooth")
                    open("int_generator_3_10/smooth.dat", "a") do file
                         write(file,data,"\n")
                    end
             end
        ...
        if (length(vars(I[1][1])) == 1)
              ...
              if !(isone(J))
                     println("princ univariate singular")
                        open("princ_univariate_3_10/singular.dat", "a") do file
                             write(file,data,"\n")
                         end 
              else
                    println("princ univariate smooth")
                      open("princ_univariate_3_10/smooth.dat", "a") do file
                          write(file,data,"\n")
                        end
              end
             ...
             if !(isone(J))
                 println("princ multivariate singular")
                    open("princ_multivariate_3_10/singular.dat", "a") do file
                         write(file,data,"\n")
                 end 
            else
                println("princ multivariate smooth")
                    open("princ_multivariate_3_10/smooth.dat", "a") do file
                         write(file,data,"\n")
                    end
            end 
        end
                 ...
                 #println("not principal univariate")#univariate ideal with multiple generators
                   ...
                   if !(isone(J))
                     println("not principle univariate singular")
                        open("np_univariate_3_10/singular.dat", "a") do file
                             write(file,data,"\n")
                          end 
                    else
                     println("not principle univariate smooth")
                      open("np_univariate_3_10/smooth.dat", "a") do file
                          write(file,data,"\n")
                        end
                    end
        else    
            #println("not principle multivariate")#multivariate ideal with multiple generators
             ...
             if !(isone(J))
                 println("np multivariate singular")
                    open("nprinc_multivariate_3_10/singular.dat", "a") do file
                         write(file,data,"\n")
                    end 
            else
                println("not principal multivariate smooth")
                    open("nprinc_multivariate_3_10/smooth.dat", "a") do file
                         write(file,data,"\n")
                    end
             end
        end
